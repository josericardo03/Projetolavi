...
import { Legislacao } from "@/app/lib/interface";
import { PortableText } from "@portabletext/react";

async function getData() {
  const query = `*[_type == 'legislacao'] {
    title,
    content,
...
export default async function Le() {
  const data: Legislacao[] = await getData();
  console.log(data);
  return (
      ...
      {data.map((leis, idx) => (
        <div key={idx} className="bg-white rounded-lg shadow-md">
          ...
          <h4 className="text-sm text-gray-600 dark:text-gray-300 p-4">
            <PortableText value={leis.content} />
          </h4>
          <div className="p-4">
            <h4 className="font-bold mb-2">Links Relacionados:</h4>
            <ul>
              {leis.linksRelacionados.map((link, idx) => (
                <li key={idx}>
